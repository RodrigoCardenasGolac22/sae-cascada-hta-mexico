...
source("analysis/00_comun.R")   # poblacion_adulta_municipal()

COV <- "data/processed/covariates"
clues <- read_excel("data/external/clues/clues_establecimientos_salud.xlsx")
...
marco <- read_csv(file.path("data/processed/geography", "muni_idx_grafo.csv"),
...
write.csv(covariable_clues %>% filter(clues_total == 0) %>% select(muni_id),
          file.path("results/estimates", "municipios_clues_cero.csv"), row.names = FALSE)
cat("Guardado: data/processed/covariates/clues_conteo_municipal.csv, results/estimates/municipios_clues_cero.csv\n")
