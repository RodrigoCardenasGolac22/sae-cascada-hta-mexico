...
strobe <- data.frame(
  ...
  `Ubicación en el manuscrito` = c(
    ...
    "Resultados > Muestra y Figura 1 (exclusión de presión arterial no válida, embarazo y ponderador faltante, cada una con su n); Materiales y métodos > Modelo estadístico (seis registros de control de dos municipios sin pobreza publicada por CONEVAL 2020; Tabla S1). Tabla S8, panel B (faltantes por covariable y denominador)",
    ...
    "Figura 1",
    paste0("Resultados > Muestra; ", T1, " (n por año). Tabla S8, panel A (características por denominador)"),
    paste0("Resultados > Muestra; Figura 1 (flujo con el n de cada exclusión); ", T1, "; Materiales y métodos > Modelo estadístico (registros sin pobreza municipal); Tabla S8, panel B"),
